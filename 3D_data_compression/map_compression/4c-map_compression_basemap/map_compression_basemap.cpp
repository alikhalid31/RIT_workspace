...
// Usage 
// ./map_compression [datset folder] [basemap_path] [day (day to be compressed)] [resolution of octree] [output folder]

int find_change (std::string, std::string, pcl::PointCloud<pcl::PointXYZ>::Ptr, float, pcl::octree::OctreePointCloudSearch<pcl::PointXYZ>&, pcl::ExtractIndices<pcl::PointXYZ>&, 
std::vector<std::string>, std::vector<float>&, std::vector<float>&, std::vector<float>& );
...

{
    ...
    compress_frames(compress_day_path, basemap_path, resolution, output_folder);

    return (0);
}


int compress_frames (std::string compress_day_path, std::string basemap_path,  float resolution, std::string output_folder)
{
    ...
    while (getline (pose_file, line )) 
    {
        ...
        if (find_change (compress_day_path, compress_frame, change_add, resolution, octree_base_R , extract_base_R, split_line , run_time_1, run_time_2, run_time_3 ) == 0 )
        {
...
int find_change (std::string compress_day_path, std::string compress_frame, pcl::PointCloud<pcl::PointXYZ>::Ptr change_add, float resolution, 
pcl::octree::OctreePointCloudSearch<pcl::PointXYZ>& octree_base_R,   pcl::ExtractIndices<pcl::PointXYZ>& extract_base_R, std::vector<std::string> pose ,
